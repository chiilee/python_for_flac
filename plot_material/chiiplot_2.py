...
psfile = 'result2.%d.ps' % frame
pngfile = 'result2.%d.png' % frame
...
denscpt = '/home/chiilee/code/git-code/plot_material/dens.cpt'
...
# phase plot
grdimage %(densgrd)s -C%(denscpt)s -R%(xmin3)f/%(xmax3)f/%(zmin3)f/%(zmax3)f -J -P -O -K >> %(psfile)s

# temperature contours
grdcontour %(tgrd)s -C+600/1200 -W1p -J -P -R -K -O >> %(psfile)s
...
psscale -C%(denscpt)s -D4.75/%(shiftz2)f/1.5/0.15h -B200:"Density(kg/m@+3@+)":  -P -O -K >>%(psfile)s
