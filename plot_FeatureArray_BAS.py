...
fa.arraysize=(5,5)
'''
BAST = np.array([  1000,  1000,  1000,  20.5,     0,
                   1000,  1000,  1000,     0,  21.8,
                   1000,  1000,  1000,     0,  21.6,
...
#BAST, BASL = fa.BAST2L(foldername,BAST,arraysize)


picnameT='pic_1BAStime'
picnameL='pic_1BASlen'
#=====================================================
fig, ax = plt.subplots(figsize=(8,6))

heatmap = ax.pcolor(BAST, cmap=plt.cm.gist_earth, vmin=10,vmax=15)                  
...
cbar.set_label('Time of 1$^{st}$ BAS (Myrs)', fontsize=20, color='black')
plt.savefig('/home/chiilee/Pic/AFc/'+'/'+picnameT+'.png') 

#=====================================================
fig, ax = plt.subplots(figsize=(8,6))

print(BASL)
heatmap = ax.pcolor(BASL, cmap=plt.cm.gist_heat, vmin=300,vmax=500)
...
for i in range(BAST.shape[0]):
    for j in range(BAST.shape[1]):
        ax.text(j+0.5, i+0.5, format(str(BASL[i,j])),fontsize=12,
               ha="center", va="center",
               color="black" if BASL[i,j] <1380 and BASL[i,j] >900 or BASL[i,j]==0 else "white")
...
cbar = plt.colorbar(heatmap) 
cbar.set_label('subducted length at 1$^{st}$ BAS (km)', fontsize=15, color='black')
plt.savefig('/home/chiilee/Pic/AFc/'+'/'+picnameL+'.png') 
