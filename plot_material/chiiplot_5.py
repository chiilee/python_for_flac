...
edotcpt = '/home/chiilee/code/git-code/plot_material/test.cpt'
...
# axis annotation
psbasemap -JX%(width)f/%(height)f -Ba100f10/a50f10::WSne -R%(left)f/%(right)f/%(zmin)f/%(zmax)f -X0.7 -Y4 -P -K > %(psfile)s

# phase plot
grdimage %(edotgrd)s -C%(edotcpt)s -R%(xmin)f/%(xmax)f/%(zmin)f/%(zmax)f -J -P -O -K >> %(psfile)s
...
# axis annotation
psbasemap -JX%(width)f/%(height)f -Ba50f5/a25f5::wSnE -R%(left2)f/%(right2)f/%(zmin2)f/%(zmax2)f -X%(shiftx)f -P -O -K >> %(psfile)s

# viscosity plot
grdimage %(viscgrd)s -C%(visccpt)s -R%(xmin2)f/%(xmax2)f/%(zmin2)f/%(zmax2)f -J -P -O -K >> %(psfile)s
...
# temperature contours
#grdcontour %(tgrd)s -C+200/400/600/800/1000/1200 -W1p -J -P -R -K -O >> %(psfile)s
...
# temperature contours
grdcontour %(tgrd)s -C+200/400/600/800/1000/1200 -L10/1600 -W1p,- -J -P -R -K -O >> %(psfile)s

###########
# colorbar #
############

makecpt -Chot -Ic -T-16/-12/1 -Z >test.cpt

psscale -C%(edotcpt)s -D0.75/%(shiftz2)f/1.5/0.15h -B1:"log@-10@-(Starin rate)(s@+-1@+)":  -P -O -K >>%(psfile)s
psscale -C%(Ccpt2)s -D2.75/%(shiftz2)f/1.5/0.15h -B0.02:"Melt Vol":  -P -O -K >>%(psfile)s
psscale -C%(visccpt2)s -D4.75/%(shiftz2)f/1.5/0.15h -B1:"log@-10@-(Viscosity)(Pa.s)":  -P -O -K >>%(psfile)s

##########
# title  #
##########
echo %(xmin)f %(shiftz)d 14 0 1 LB "Model=%(model)s   Frame=%(frame)d   Trench location=%(xtrench).3f km   Time=%(modeltime).2f Myr " | \
